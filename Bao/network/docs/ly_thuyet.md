...
**CIDR**
CIDR (Classless Inter-Domain Routing - Định tuyến liên miền không phân lớp) là một phương pháp phân bổ địa chỉ IP và định tuyến hiệu quả, giúp tối ưu hóa không gian địa chỉ IPv4 và giảm gánh nặng cho các bảng định tuyến trên Internet.
Nó thay thế hệ thống phân lớp cũ (Class A, B, C) bằng cách sử dụng ký hiệu dấu gạch chéo (/) để xác định linh hoạt phần mạng và phần host.

Các đặc điểm chính của CIDR:
Ký hiệu CIDR: Ví dụ 192.168.1.0/24, trong đó /24 cho biết 24 bit đầu tiên được dùng cho Network ID (tương đương Subnet Mask: ).
Siêu mạng (Supernetting): Cho phép gộp nhiều mạng nhỏ thành một khối lớn, giúp tối ưu hóa không gian địa chỉ.
Linh hoạt: Không bị giới hạn bởi các lớp mạng A, B, hay C, giúp quản trị viên chia mạng con (
) chính xác với nhu cầu.
Ứng dụng: Rất quan trọng trong điện toán đám mây (như tạo VPC trên Amazon Web Services (AWS)) và quản lý mạng doanh nghiệp

**Khi muốn chia nhỏ mạng**

Tăng prefix_length lên → mạng nhỏ hơn
Mỗi lần tăng +1 bit → số subnet gấp đôi, số host giảm một nửa

**ví dụ**

chia mạng 192.168.1.0/24 thành 4 mạng nhỏ hơn /24 -> /26
...
**Các bước ứng dụng VLAN trên máy tính (Windows)**

Truy cập Device Manager: Nhấn chuột phải vào Start -> chọn Device Manager.
Mở Properties của NIC: Mở mục Network adapters, nhấp chuột phải vào card mạng (ví dụ: Intel Ethernet, Realtek...) và chọn Properties.
Thiết lập VLAN ID:
Chuyển sang tab Advanced (Nâng cao).
Tìm mục VLAN ID hoặc VLAN & Priority.
Nhập ID VLAN (ví dụ: 10, 20...) mà bạn muốn máy tính tham gia vào ô Giá trị (Value).
Lưu cấu hình: Nhấn OK để áp dụng thay đổi. Card mạng sẽ tự thêm thẻ 802.1Q vào các gói tin đi ra

**Các bước cấu hình VLAN trên Ubuntu (Netplan)**

- Cài đặt vlan package: Chạy lệnh `sudo apt install vlan.`
- Xác định interface mạng: Sử dụng lệnh `ip a` để tìm tên card mạng (ví dụ: `enp3s0`).
- Cấu hình Netplan: Chỉnh sửa file yaml trong `/etc/netplan/` (ví dụ `01-netcfg.yaml`):
...
network:
  ...
  renderer: networkd
  ethernets:
    enp3s0: # Giao diện cha
      dhcp4: no
  vlans:
    vlan10: # Tên VLAN interface
      id: 10 # ID VLAN
      link: enp3s0
      addresses: [192.168.10.10/24] # IP của VLAN
      gateway4: 192.168.10.1
```

- Áp dụng cấu hình: Chạy `sudo netplan apply`

**Lưu ý quan trọng**
Cấu hình Switch: Cổng trên switch kết nối với máy tính phải được cấu hình ở chế độ Trunk (cho phép mang nhiều thẻ VLAN) thay vì Access.
Driver: Máy tính cần cài đặt driver card mạng hỗ trợ VLAN (thường là card mạng Intel hoặc các loại card chuyên dụng).
Ứng dụng: VLAN giúp cách ly mạng (ví dụ: chia VLAN cho nhân viên, camera, khách) trên cùng một hạ tầng vật lý
...
**Đặc điểm chính của định tuyến tĩnh**

Cấu hình thủ công: Các tuyến đường được nhập vào bảng định tuyến bởi quản trị viên.
Không tự động cập nhật: Nếu đường truyền bị đứt, tuyến đường tĩnh không tự đổi hướng (trừ khi cấu hình thêm backup).
Phù hợp mạng nhỏ/đơn giản: Lý tưởng cho các mạng có ít Router và đường truyền ổn định.
Tiết kiệm tài nguyên: Không tốn băng thông để trao đổi thông tin cập nhật giữa các router như định tuyến động.
Default Route (Tuyến mặc định): Là một dạng đặc biệt của định tuyến tĩnh (0.0.0.0/0), dùng khi router không có đường đi cụ thể trong bảng.

Ưu điểm:
Đơn giản, dễ cấu hình: Không cần cấu hình giao thức phức tạp.
Bảo mật: Quản trị viên kiểm soát hoàn toàn đường đi của dữ liệu.
Hiệu quả: Không tốn tài nguyên CPU/RAM và băng thông mạng để chạy giao thức.

Nhược điểm:
Kém linh hoạt: Khi mạng thay đổi (thêm node, đứt link), cần cấu hình lại thủ công.
Không quy mô: Không khả thi trong mạng lớn, phức tạp.

_Ví dụ câu lệnh cấu hình trên Router Cisco:_
Router(config)# ip route [mạng*đích] [subnet_mask] [next-hop_IP_hoặc_interface_ra]

_Ví dụ: ip route 192.168.2.0 255.255.255.0 10.0.0.2 (Gửi dữ liệu đến mạng 192.168.2.0 thông qua địa chỉ 10.0.0.2)._
...
**Các thông tin chính về NAT:**
Vị trí hoạt động: Thường nằm trên router biên (border router) hoặc tường lửa (firewall) kết nối giữa mạng LAN và Internet.
Nguyên lý hoạt động: Khi thiết bị nội bộ gửi dữ liệu ra ngoài, NAT thay đổi IP nguồn thành IP công cộng của router. Khi dữ liệu phản hồi về, NAT thay đổi IP đích lại thành IP riêng của thiết bị gốc.

**Phân loại:**
Static NAT: Ánh xạ 1-1 giữa IP riêng và IP công cộng.
Dynamic NAT: Ánh xạ 1-1 nhưng chọn từ một nhóm IP công cộng.
PAT (NAT Overload): Ánh xạ nhiều IP riêng thành 1 IP công cộng duy nhất, phân biệt qua cổng (port).

Ưu điểm: Tiết kiệm không gian địa chỉ IPv4, giấu địa chỉ IP nội bộ giúp bảo mật hơn.
Nhược điểm: Làm tăng độ trễ (latency), một số ứng dụng thời gian thực có thể bị ảnh hưởng

**Cơ chế và cách hoạt động của NAT**
Gửi yêu cầu (Inside -> Outside): Khi thiết bị nội bộ (ví dụ: 192.168.1.10) gửi gói tin ra Internet, Router NAT chặn lại, thay đổi địa chỉ IP nguồn thành IP công cộng của nó (ví dụ: 200.0.0.1).
Lưu vết (NAT Table): Router ghi lại sự ánh xạ này vào bảng NAT (NAT Translation Table) để biết gói tin trả về thuộc về thiết bị nào.
Nhận phản hồi (Outside -> Inside): Khi Internet gửi dữ liệu về, Router tra bảng NAT, thay đổi IP đích từ IP công cộng thành IP nội bộ gốc và chuyển tiếp dữ liệu.
